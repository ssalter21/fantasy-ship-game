...
run_finish_ship_battle_halts_the_encounter_when_the_captain_took_leave_combat :: proc(t: ^testing.T) {
	...
	outcome, _ := run_finish_ship_battle(&battle)
...
@(test)
run_finish_ship_battle_payout_above_capacity_falls_overboard :: proc(t: ^testing.T) {
...
@(test)
run_finish_ship_battle_a_kill_of_a_broke_opponent_pays_nothing :: proc(t: ^testing.T) {
...
@(test)
run_status_is_won_when_the_ship_reaches_haven_with_positive_hull :: proc(t: ^testing.T) {
	s := ship.Ship{hull = 1}
...
@(test)
run_status_is_lost_when_hull_reaches_zero_even_at_the_haven :: proc(t: ^testing.T) {
	s := ship.Ship{hull = 0}
	haven := Node{kind = .Haven}
...
@(test)
run_status_is_in_progress_away_from_haven_with_positive_hull :: proc(t: ^testing.T) {
	s := ship.Ship{hull = 20}
