...
// stats so a deeper fight isn't Hull-pool-only: voyage_fight_opponent_hull is the
// opponent's Hull baseline, voyage_fight_opponent_durability its flat
...
// voyage_reward_cargo is the Reward primitive's stakes reading: the cargo a
...
// voyage_status reports the voyage's outcome: lost at 0 Hull (permadeath) regardless
// of position, won by being at Haven with Hull > 0, otherwise still in progress.
// Hull loss itself happens in core/combat/core/ship; this is just the voyage-level
// read of that state.
