...
// a Ghost_Snapshot (ADR-0008) of the player-side ship at that point. The stage's
// tuned magnitudes were already baked into its content at generation time
// (content.odin); this is only the arrival-time application and snapshot.
...
// perspective. difficulty_rating is recomputed from zone/depth rather than read
// off the opponent's (now battle-worn) hp, since that would reflect remaining HP,
// not the node's original tuned stakes. (The field is itself retired as a
// misnomer by ADR-0014; issue #130 spends that.) The returned snapshot's layout
// aliases s (see run_ghost_snapshot_of): the Sim owns the single arena-backed
// capture (issue #82).
run_finish_ship_battle :: proc(battle: ^combat.Battle, s: ^ship.Ship, fight: ^Stage_Fight, zone: Zone, steps: int) -> Ghost_Snapshot {
	...
	return run_ghost_snapshot_of(s, steps, zone, run_ship_battle_difficulty(Scaling_Site{zone = zone, depth = fight.depth}))
}
...
// post-trade Ghost_Snapshot (ADR-0008); the trade's own gain_durability is
// already this node's stakes-scaled tuned magnitude, so it doubles as the
// snapshot's difficulty_rating — the very mismatch that retires that field as a
// misnomer (ADR-0014, issue #130).
run_apply_stat_trade :: proc(s: ^ship.Ship, trade: Stage_Trade, zone: Zone, steps: int) -> Ghost_Snapshot {
