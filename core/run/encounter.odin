...
// application. voyage_start_battle/voyage_finish_ship_battle bracket a Fight around
...
// (voyage_encounter_resolve_stage in stage.odin), driven by the Sim (issue #131).

// voyage_start_battle triggers a Fight stage: hands off to core/combat's existing
...
// resolving the stage at all, so the caller checks voyage_can_travel before it
...
// the player's purse exactly like a Reward (voyage_apply_reward). A fled opponent and
...
// old voyage_apply_upgrade_offer / its resolve-time Ghost_Snapshot are retired with
...
// voyage_trade_stat_reading is how much of `stat` the ship currently has, for
...
// voyage_trade_pay deducts a trade's cost. For the base-field stats the amount is
// measured against the effective stat (voyage_trade_can_accept) but *paid out of the
// base field*, which is the only field a voyage owns — so a heavily-fitted ship can
...
// voyage_trade_grant applies a trade's gain. Gaining HP is a repair and so caps at
...
// voyage_apply_trade resolves an **accepted** Trade stage (issue #136), permanently
// swapping the trade's cost for its gain.
//
// Only accepting reaches here. The old voyage_apply_stat_trade applied on arrival —
...
// when voyage_trade_can_accept, so arriving here unable to pay is a driver bug.
...
// voyage_apply_reward pays a Reward stage's treasure into the ship's purse (issues
// #132, #133).
//
// The amount is not computed here — it was baked into the stage at generation
// (voyage_bake_stage), so this is only the application, like every other proc in this
// file.
//
// Unconditional, and that is the primitive: a Reward is a boon with nothing to
// decline, so unlike voyage_apply_trade there is no affordability gate to assert and
