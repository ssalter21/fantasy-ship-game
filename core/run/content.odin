...
hostile_roster := [?]Hostile_Archetype {
	...
	// voyage_fit_hostile_loadout). War Hound's own magnitude is gated below half HP, so
...
// voyage_make_opponent_ship computes a Ship Battle opponent's stakes-scaled stats (hp,
...
voyage_make_opponent_ship :: proc(site: Scaling_Site) -> ship.Ship {
	hp := voyage_fight_opponent_hp(site)
	return ship.Ship{
		hp         = hp,
		max_hp     = hp,
