...
@(test)
applying_a_stat_trade_emits_an_encounter_resolved_event_with_a_post_trade_snapshot :: proc(t: ^testing.T) {
	s := ship.Ship{hp = 20, max_hp = 20, durability = 2, speed = 5}
	trade := Encounter_Stat_Trade{gain_durability = 3, cost_speed = 1}

	events: [dynamic]Event
	defer delete(events)
	run_apply_stat_trade(&s, trade, .Open_Sea, 4, &events)
	defer delete(events[0].(Event_Encounter_Resolved).snapshot.ship.layout)

	testing.expect_value(t, len(events), 1)
	resolved, ok := events[0].(Event_Encounter_Resolved)
	testing.expect(t, ok)
	testing.expect_value(t, resolved.snapshot.ship.durability, 5) // post-trade, not pre-trade
	testing.expect_value(t, resolved.snapshot.ship.hp, 20)
	testing.expect_value(t, resolved.snapshot.progress.steps, 4)
	testing.expect_value(t, resolved.snapshot.progress.zone, Zone.Open_Sea)
	testing.expect_value(t, resolved.snapshot.progress.difficulty_rating, 3)
}

@(test)
resolving_an_upgrade_offer_emits_an_encounter_resolved_event_with_the_offers_quality_as_difficulty_rating :: proc(t: ^testing.T) {
	s := ship.Ship{hp = 20, max_hp = 20, durability = 2, speed = 5}
	offer := Encounter_Upgrade_Offer{quality = 30}

	events: [dynamic]Event
	defer delete(events)
	run_apply_upgrade_offer(&s, offer, .Open_Sea, 6, &events)
	defer delete(events[0].(Event_Encounter_Resolved).snapshot.ship.layout)
	...
	testing.expect_value(t, resolved.snapshot.ship.durability, 2)
	testing.expect_value(t, resolved.snapshot.ship.speed, 5)
	testing.expect_value(t, resolved.snapshot.ship.hp, 20)
	testing.expect_value(t, resolved.snapshot.progress.steps, 6)
	testing.expect_value(t, resolved.snapshot.progress.zone, Zone.Open_Sea)
	testing.expect_value(t, resolved.snapshot.progress.difficulty_rating, 30)
}
...
finishing_a_ship_battle_emits_an_encounter_resolved_event_with_a_snapshot_of_the_players_ship :: proc(t: ^testing.T) {
	...
	events: [dynamic]Event
	defer delete(events)
	run_finish_ship_battle(&battle, &player, &encounter, .Deep, 8, &events)
	defer delete(events[0].(Event_Encounter_Resolved).snapshot.ship.layout)

	testing.expect_value(t, len(events), 1)
	resolved, ok := events[0].(Event_Encounter_Resolved)
	testing.expect(t, ok)
	testing.expect_value(t, resolved.snapshot.ship.hp, 20) // player's own max_hp, not the opponent's
	testing.expect_value(t, resolved.snapshot.progress.steps, 8)
	testing.expect_value(t, resolved.snapshot.progress.zone, Zone.Deep)
	testing.expect_value(t, resolved.snapshot.progress.difficulty_rating, run_ship_battle_difficulty(.Deep, 2))
}
...
finishing_a_ship_battle_that_has_not_ended_asserts :: proc(t: ^testing.T) {
	...
	run_finish_ship_battle(&battle, &player, &encounter, .Coastal, 0, &events)
}
