...
// lost its gainer when Braced Bulkheads left), and Max HP / Cargo sit on both
...
the_trade_roster_covers_the_stats_the_surviving_three_rows_can :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, gained, bit_set[Trade_Stat]{.HP, .Max_HP, .Cargo})
	testing.expect_value(t, cost, bit_set[Trade_Stat]{.Max_HP, .Durability, .Cargo})
}
