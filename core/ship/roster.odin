...
ship_item_roster :: proc() -> [ITEM_ROSTER_SIZE]Roster_Item {
	return {
		roster_item(.Splash, "Deckhands", .Small, 3, {.Crew}, {effect_phase_contribution(expr_const(2))}),
		...
		roster_item(.Splash, "Oakum & Pitch", .Medium, 24, {.Artifact}, {effect_repair(expr_const(3))}),
		roster_item(.Splash, "Spare Timbers", .Small, 9, {.Cargo}, {effect_repair(expr_const(2))}),
		roster_item(.Splash, "Spare Rigging", .Small, 5, {.Artifact}, {effect_modify_speed(expr_const(1))}),
		roster_item(.Splash, "Salt Provisions", .Small, 7, {.Cargo}, {effect_repair(expr_const(2))}),
		roster_item(.Splash, "Carpenter's Mate", .Small, 5, {.Crew}, {effect_repair(expr_const(1))}),
		roster_item(.Splash, "Deck Pumps", .Medium, 20, {.Artifact}, {effect_repair(expr_const(2))}),
		roster_item(
			.Splash,
			"Powder Monkeys",
			.Small,
			6,
			{.Crew},
			{effect_phase_contribution(expr_const(1), Selector(Slot_Size.Small))},
		),
		roster_item(
			.Splash,
			"Smuggler's Crates",
		...
		roster_item(.Splash, "War Hound", .Small, 7, {.Beast}, {effect_phase_contribution(expr_below_hull_percent(50, 4))}),
		roster_item(.Splash, "Lookout Nest", .Small, 5, {.Crew}, {effect_phase_contribution(expr_while_opponent_faster(2))}),
		roster_item(.Splash, "Bilge Rats", .Small, 5, {.Beast}, {effect_phase_contribution(expr_from_round(3, 3))}),
		...
		roster_item(.Shallow, "Shipwright's Kit", .Medium, 25, {.Artifact}, {effect_repair(expr_const(4))}),
		roster_item(.Shallow, "Copper Sheathing", .Medium, 16, {.Artifact}, {effect_modify_speed(expr_const(2))}),
		roster_item(.Shallow, "Ship's Surgeon", .Medium, 16, {.Crew}, {effect_repair(expr_const(3))}),
		roster_item(
			.Shallow,
			"Outriggers",
			.Small,
			5,
			{.Artifact},
			{effect_modify_speed(expr_const(1)), effect_phase_contribution(expr_const(1))},
		),
		roster_item(
			.Shallow,
			"Gun Captain",
		...
		roster_item(
			...
			20,
			{.Cargo},
			{effect_phase_contribution(expr_const(1), Selector(Tag.Cargo))},
		),
		...
		roster_item(
			...
			15,
			{.Artifact},
			{effect_phase_contribution(expr_while_opponent_slower(8))},
		),
		roster_item(
			.Shallow,
			"Chain & Bar Shot",
		...
		roster_item(.Deep, "Dragon Turtle", .Large, 40, {.Beast}, {effect_repair(expr_const(6))}),
		roster_item(.Deep, "Adamant Sigil", .Medium, 25, {.Artifact}, {effect_repair(expr_const(5))}),
		roster_item(.Deep, "Enchanted Keel", .Medium, 15, {.Artifact}, {effect_modify_speed(expr_const(3))}),
		roster_item(.Deep, "Titan's Heart", .Large, 36, {.Artifact}, {effect_repair(expr_const(5))}),
		roster_item(.Deep, "Shipwright's Stores", .Medium, 22, {.Cargo}, {effect_repair(expr_const(4))}),
		roster_item(
			.Deep,
			"Admiral's Guard",
			.Medium,
			17,
			{.Crew},
			{effect_phase_contribution(expr_const(1), Selector(Tag.Crew))},
		),
		roster_item(
			.Deep,
			"Broadside Master",
			...
			36,
			{.Crew, .Weapon},
			{effect_phase_contribution(expr_const(1), Selector(Tag.Weapon)), effect_phase_contribution(expr_const(2))},
		),
		roster_item(
			.Deep,
			"Hunter's Pack",
			...
			18,
			{.Beast},
			{effect_phase_contribution(expr_const(1), Selector(Tag.Beast))},
		),
		roster_item(
			.Deep,
			"Flagship Colors",
		...
		roster_item(
			...
			6,
			{.Artifact},
			{effect_phase_contribution(expr_const(1), Selector(Visibility.Concealed)), effect_phase_contribution(expr_const(1))},
		),
		roster_item(
			.Deep,
			"Wraith Cannon",
		...
		roster_item(.Deep, "Siege Battery", .Large, 44, {.Weapon}, {effect_phase_contribution(expr_from_round(5, 22))}),
