...
fight_opponent_hull_rises_by_zone_and_depth :: proc(t: ^testing.T) {
	expect_rises_by_zone_and_depth(t, voyage_fight_opponent_hull)
}
...
reward_cargo_rises_by_zone_and_depth :: proc(t: ^testing.T) {
	...
	expect_rises_by_zone_and_depth(t, voyage_reward_cargo)
}
...
a_reward_outpays_selling_a_stat_at_the_same_site :: proc(t: ^testing.T) {
	...
	for zone in Zone {
		for depth in 0 ..= DEPTH_STEPS {
			...
			testing.expectf(
				t,
				voyage_reward_cargo(site) > voyage_trade_swing(site.zone, .Cargo),
...
@(test)
voyage_map_create_has_fifty_nodes_plus_start_and_haven :: proc(t: ^testing.T) {
	for seed in TEST_SEEDS {
		m := voyage_map_create(seed)
		defer voyage_map_destroy(&m)
...
@(test)
voyage_map_create_has_exactly_one_start_and_one_haven_neither_belonging_to_a_zone :: proc(t: ^testing.T) {
...
@(test)
voyage_finish_ship_battle_halts_the_encounter_when_the_captain_took_break_off :: proc(t: ^testing.T) {
	player := ship.Ship{hull = 20, max_hull = 20, speed = 5}
	fight := Stage_Fight{opponent = ship.Ship{hull = 10, speed = 3}}
...
@(test)
voyage_apply_trade_moves_cargo :: proc(t: ^testing.T) {
...
voyage_apply_trade_pays_before_granting_so_a_repair_caps_against_the_sold_ceiling :: proc(t: ^testing.T) {
	...
	voyage_apply_trade(&s, trade_of(.Hull, 8, .Max_Hull, 6))
...
@(test)
voyage_apply_trade_gaining_max_hull_raises_the_ceiling_without_filling_it :: proc(t: ^testing.T) {
	s := ship.ship_starting_ship()
	defer delete(s.layout)
	s.hull = 12
	s.max_hull = 20

	voyage_apply_trade(&s, trade_of(.Max_Hull, 5, .Cargo, 15))
...
@(test)
voyage_status_is_won_when_the_ship_reaches_haven_with_positive_hull :: proc(t: ^testing.T) {
	s := ship.Ship{hull = 1}
	haven := Node{kind = .Haven}

	testing.expect_value(t, voyage_status(&s, haven), Voyage_Status.Won)
}

@(test)
voyage_status_is_lost_when_hull_reaches_zero_even_at_the_haven :: proc(t: ^testing.T) {
	s := ship.Ship{hull = 0}
	haven := Node{kind = .Haven}

	testing.expect_value(t, voyage_status(&s, haven), Voyage_Status.Lost)
}

@(test)
voyage_status_is_in_progress_away_from_haven_with_positive_hull :: proc(t: ^testing.T) {
	s := ship.Ship{hull = 20}
...
@(test)
voyage_can_travel_is_false_once_hull_reaches_zero :: proc(t: ^testing.T) {
