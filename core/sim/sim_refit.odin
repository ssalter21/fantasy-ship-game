...
// the surviving cargo is re-stowed into whatever capacity remains: everything
...
// cargo and neither re-stows.
//
// `cargo_before` is the cargo snapshotted *before* the placement, so a hold
// a Replace displaces has its cargo flow into the remaining slots rather than
...
// reaches a refit (it is stowed by ship_stow_cargo, not fitted), and re-laying
// cargo_before would double-count a cargo incoming's own stack.
