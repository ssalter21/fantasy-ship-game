...
scripted_player_command_travels_to_a_legal_forward_neighbor_of_the_current_node :: proc(t: ^testing.T) {
	...
	cmd := scripted_player_command(m, 0, options, .Awaiting_Travel_Choice)
...
@(test)
scripted_player_command_answers_every_non_travel_phase :: proc(t: ^testing.T) {
	// The non-travel branches read none of the travel inputs, so zero values do.
	battle := scripted_player_command(voyage.Map{}, 0, nil, .Awaiting_Battle_Command)
	...
	options := scripted_player_command(voyage.Map{}, 0, nil, .Awaiting_Option_Choice)
	...
	trade := scripted_player_command(voyage.Map{}, 0, nil, .Awaiting_Trade_Choice)
	...
	refit := scripted_player_command(voyage.Map{}, 0, nil, .Awaiting_Refit)
