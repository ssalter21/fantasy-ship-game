// Package presentation is the game's whole rendering side — the screens, the
// session loop and capture mode — importable by any executable (#433). Its
// exported surface is deliberately small: run, capture_requested and
// capture_main; every screen and the state record behind them are package-
// private. Nothing under core/ may import it (ADR-0003).
...
// run is the player-facing session, boot to quit: one window, and above run_session
// the loop that makes a session N voyages (ADR-0022 — hosted here so an executable
// importing presentation gets the whole session, not screens to reassemble).
// run_session stays the voyage's single driver loop, called once per voyage; the
// Chart Table sits above it, over no Sim at all.
run :: proc() {
...
Game_State :: struct {
	...
	// framing (#476). The travel plays once per *stop*, not once per loop call: a Shop re-enters
	// offer_shop_loop after every buy with a refilled shelf, and swinging round again each time
	// would put 0.9 seconds of scenery between a captain and their second purchase. Cleared by
	// Event_Stage_Entered, which is exactly "a new stop", so the next stage travels afresh. The
	// in-flight tween itself is a loop local — it runs to completion inside one call.
...
dispatch :: proc(data: rawptr, event: sim.Event) {
	...
	case sim.Event_Stage_Entered:
		...
		// A new stop is the one thing that makes an Offer or Shop travel to its framing again
		// (#476) — a buy inside the same stop re-enters its loop and must not replay the move.
		state.stage_alongside = false
