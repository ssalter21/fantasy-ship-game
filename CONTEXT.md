...
- **Node** — a single location in the run's map. The map is a procedurally-generated connected graph, regenerated fresh each run (seeded, reproducible); a node carries **edges** to other nodes and, if it's an encounter, a stage list whose content is hidden until arrival unless its **first** stage is a revealing one.
  _Avoid_: point, tile.
...
- **Landmark** — Start or Goal: a node carrying no encounter (never triggers one) and always fully visible on the map. Port left this category in ADR-0014 — it is an encounter (`[Shop]`) that is visible because it opens on a Shop, not because it is exempt.
