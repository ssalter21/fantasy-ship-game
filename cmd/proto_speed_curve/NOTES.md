...
## The gap is now closed (#176): the straddle is not a roster property

The gap below is **filled** — `main.odin` now sweeps the player's purse against the flat
50% roster, and the answer changes what the straddle test can assert.

**The straddle holds only while the player's purse is between 20 and 65, of a possible
0–90.** Outside that window it fails in *both* directions, and both are #135's
flat-`FIGHT_OPPONENT_SPEED` bug wearing a different hat:

| player purse | P speed | slower / faster | |
|---|---|---|---|
| 0–15 (broke) | 9–8 | 8 / 0 | player outruns the whole roster — nothing can ever flee |
| **20–65** | **7–3** | **5/2 … 1/7** | **straddles** |
| 70–90 (rich) | 2–0 | 0 / 7–8 | the whole roster outruns the player — Leave Combat is dead |

So **"does the roster straddle 4?" is no longer a well-formed question.** Under the weight
model the player's Speed is not 4; it is whatever their purse says. Straddle is a joint
property of (roster, player purse), and any test that pins one side must pin the other
explicitly rather than inheriting `STARTING_SPEED`.

**The sharp edge: the starting purse of 50 sits inside the window, but a first Reward
leaves it.** Rewards pay 20–45, so one payday puts the player at 70–95 — past 65, into the
dead zone where every hostile bolts and none can be caught (only a wreck pays, #159). The
window is off-centre: 30 of room below the starting purse, **15 above**. Whether falling
out of the window is the model working (getting rich makes you catchable) or the model
breaking is **#177's** question, not this prototype's.

## Known gap (CLOSED — see above)
