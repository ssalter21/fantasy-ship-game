...
// The dev's provisional call on #176: a hostile's cargo fittings are all half full —
// neither authored per archetype (#158) nor scaled by depth (#159), until hostile ship
// templates exist to derive richness from.
HOSTILE_FLAT_LADEN_PERCENT :: 50

// The divisor #158 adopted; the purse sweep is run at the real exchange rate only.
PLAYER_SWEEP_DIVISOR :: 10
...
main :: proc() {
	...
	// ---- #176: the straddle against the PLAYER's purse ---------------------------
	//
	// Every table above pins the player at their starting weight, so each "straddles
	// 4?" verdict assumes a captain who never spends and never earns. Under the flat
	// 50% fill the hostile side is static — but the player's is not, and the whole
	// destination is that getting rich makes you catchable. So sweep the purse.
	curve := calibrate(PLAYER_SWEEP_DIVISOR, player_start_weight)
	fmt.printf(
		"=== #176: hostiles flat %d%% full, player purse swept 0..%d ===\n",
		HOSTILE_FLAT_LADEN_PERCENT,
		player.cargo_capacity,
	)
	fmt.printf("curve: speed = %d + mods - weight/%d\n", curve.base, curve.divisor)
	fmt.println()
	...
	for purse := 0; purse <= player.cargo_capacity; purse += 5 {
		player_speed := curve_speed(curve, player.fitting_weight + purse, player.speed_modifier)
		slower, faster := 0, 0
		for b in builds {
			sp := curve_speed(curve, weight_at(b, HOSTILE_FLAT_LADEN_PERCENT), b.speed_modifier)
			if sp < player_speed {
				slower += 1
			}
			if sp > player_speed {
		...
		verdict := "NO  - player outruns the whole roster"
		if slower > 0 && faster > 0 {
			verdict = "yes"
		} else if faster > 0 {
			verdict = "NO  - the whole roster outruns the player"
		}
		marker := " "
		if purse == ship.STARTING_TREASURE {
			marker = "*"
		}
		fmt.printf(
			"%s%-8s %8s %8s %8s %8s   %s\n",
			marker,
			pad(purse, 8),
			pad(player.fitting_weight + purse, 8),
			pad(player_speed, 8),
			pad(slower, 8),
			pad(faster, 8),
			verdict,
	...
	fmt.println("  (* = the starting purse. slower/faster count hostiles against the PLAYER's")
	fmt.println("   speed at that purse, not against a fixed 4.)")
}
