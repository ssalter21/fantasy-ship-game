...
a_fight_row_carries_one_field_per_header_column_in_that_order :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, line, "12,2,deep,3,Ironclad Hulk,4,destroyed,player,90,25,120,120,fire,1,55")
...
a_row_names_how_the_battle_ended_apart_from_who_it_went_to :: proc(t: ^testing.T) {
	...
	testing.expect_value(t, headless_winner_name(combat.Side.A), "player")
	testing.expect_value(t, headless_winner_name(combat.Side.B), "hostile")
	testing.expect_value(t, headless_winner_name(nil), "none")
}
...
a_sweep_writes_the_fight_rows_of_the_voyages_it_ran :: proc(t: ^testing.T) {
	rows, fights, ok := sweep_test_texts(Run_Request{seed = 1, runs = 6}, "sweep-fights.csv")
