...
- **A single saturated warm** — held back and always *small*, against a field that is otherwise cool or
  desaturated. The guide spends it as coral-red `#E1552B`, reserved for danger and the chart's X. Its scarcity
  is the point. The daylight set carries this as terracotta roofs and warm cliff edges, not as an accent on a
  dark ground: the two night scenes that once witnessed the relationship are retired (see the table below).
...
> depth ramp; it is a **bright daylight world with one scarce saturated warm**, and the guide now derives from
> the daylight images directly. The amber accent that reading produced went with it: the guide's "Controls do
> not have a signal colour" retired it, and no roster tone replaced it.
...
| `ship-night.jpg` | **Nothing. Retired.** | Witnessed one thing — small warm lantern points against a large cold field — and that relationship was the amber accent, which the style guide no longer carries. A night scene on a navy ground the palette has moved off: **do not read colour of any kind out of it.** |
| `ship-battle.jpg` | **Nothing. Retired.** | Same relationship as `ship-night`, louder (muzzle flare on blue), and retired for the same reason. Watermarked (Magnific). |
