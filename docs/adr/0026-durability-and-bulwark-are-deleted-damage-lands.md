...
- **The retired vocabulary is recorded, never un-said.** ADR-0004/0006 still describe a Durability stat and a subtracting damage formula; read those as arithmetic that existed and was deleted here. CONTEXT.md drops the Durability and bulwark entries and files both under the `_Avoid_` convention (ADR-0021). ADR-0006's Status line and its damage clause carry a back-pointer here — the precedent ADR-0004 set when ADR-0012 and ADR-0020 landed on it — so a reader arriving at 0006 first is not shown the deleted formula as live decision. The reasoning prose around it is left standing.
