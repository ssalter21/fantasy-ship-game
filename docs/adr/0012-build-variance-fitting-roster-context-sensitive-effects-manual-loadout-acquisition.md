...
Accepted. Amends ADR-0004. ADR-0004's core model — a fixed list of sized/visibility-tagged slots, a single unified **fitting** concept, and the exact-size fit rule — carries forward unchanged. This ADR retires ADR-0004's explicit deferral of "a fitting roster beyond the 3 starting fittings plus their upgraded variants" and extends the fitting model along the axes that let a run's ship diverge from every other run's. It also builds on ADR-0006 (fitting categories / phased combat) and ADR-0008 (the Ghost_Snapshot must be able to carry a fitting's effect as data). **Amended by ADR-0020** (the weight economy) on two points: this ADR's **stat-modifier effect** on Speed now modifies a term in a *derived* stat (`base + modifiers − weight/10`) rather than adjusting a base field, so `Modify_Speed` items add to `modifiers` and weight subtracts alongside them; and this ADR's **Port shop** spend "deducts from the ship's `starting_treasure`" — that field is **deleted** by ADR-0020, so a shop's money now leaves the ship's cargo holds rather than a scalar field (the exact spend-under-weight interaction is left to the migration, since a purchase makes the ship lighter and heavier at once). See ADR-0020. **Amended by ADR-0031** on the Item Offer: an offered item is the roster line unmodified, and depth moves the tier band a node draws from rather than the items' magnitudes.
