...
An encounter is **visible on the map iff its first stage reveals** — today only `Shop` reveals. (This ADR originally said "iff it contains a revealing stage"; **ADR-0016 corrected it to the first stage**, which is what the consequence below always assumed.) Visibility stops being a `Node_Kind` property and becomes a question asked of the stage list. ADR-0009's Variant A presentation is unchanged in every other respect: the graph is always drawn; a non-revealing encounter's content stays hidden until arrival.
...
- ADR-0009's presentation gains one wrinkle: visibility is now asked of an encounter's stages, so **the renderer can no longer key off landmark-ness** — it asks the stage list, exactly as the Sim's mask does. (This bullet originally continued "so a visible non-Port node (a merchant vessel) is possible"; **ADR-0016 made that false** — a merchant does not open on its Shop, so it does not reveal. The renderer's obligation survives the correction: `Node_Kind.Port` is gone (ADR-0015), so there is no landmark-ness left to key off even though what reveals is once again only Ports.)
...
- Out of scope for this effort and unchanged by it: the Item Offer redesign, economy tuning (ADR-0013's cost escalation shipped in #124 and carries over untouched — the Shop stage bakes the same per-purchase surcharge), branch graphs (stages advance by cursor; no per-outcome successor tables — the gate field can widen later without restructuring callers), and runtime trait rolling.
